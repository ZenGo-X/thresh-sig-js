...
pub fn p2_generate_master_key(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    let expected_args = 2;
...
pub fn p2_get_child_share(mut cx: FunctionContext) -> JsResult<JsString> {
    let expected_args = 3;
    if cx.len() != expected_args {
        return cx.throw_error("Invalid number of arguments");
    }
...
pub fn p2_sign(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    let expected_args = 7;
    if cx.len() != expected_args {
        return cx.throw_error("Invalid number of arguments");
    }
...
impl Task for SignTask {
    ...
    fn perform(&self) -> Result<Self::Output, Self::Error> {
        ...
        let signature = sign(&client_shim, self.msg_hash.clone(), &self.mk, self.x.clone(), self.y.clone(), &self.id);
